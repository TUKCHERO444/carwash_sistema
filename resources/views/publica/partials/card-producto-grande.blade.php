{{--
    Card de producto grande (sección de productos por categoría).
    Props:
      - $producto  : App\Models\Producto con marca cargada.
      - $categoria : (opcional) Categoria; si no se pasa se usa la del producto.
      - $junta     : (opcional, bool) true si va dentro de una tabla junta
                     (sin borde/redondeo propios para unirse a la celda).
    Enlaza al detalle público del producto.
--}}
@php
...
<article class="group {{ $junta ? 'h-full bg-white' : 'rounded-2xl border border-steel-200 bg-white shadow-sm' }} flex flex-col overflow-hidden transition-shadow hover:shadow-md">
