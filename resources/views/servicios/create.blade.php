...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-text-primary-dark">Crear servicio</h1>
    ...
    <div class="bg-surface rounded-lg border border-main p-6 max-w-lg">
        <form action="{{ route('servicios.store') }}" method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="mb-5">
                ...
                >
                    <option value="sparkles" @selected(old('icono', 'sparkles') === 'sparkles')>Sparkles</option>
                    @foreach($iconos as $opcion)
                        @if($opcion !== 'sparkles')
                            <option value="{{ $opcion }}" @selected(old('icono') === $opcion)>{{ ucfirst($opcion) }}</option>
                        @endif
                    @endforeach
                </select>
                @error('icono')
            ...
            <div class="flex items-center gap-3">
                <button
                    type="submit"
                    aria-label="Guardar nuevo servicio"
