...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-text-primary-dark">Detalle del Lavado</h1>
    ...
    <div class="bg-surface rounded-lg border border-main p-6 mb-6">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-primary">{{ $lavado->fecha->format('d/m/Y') }}</dd>
            </div>
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-primary">
                    {{ $lavado->automotor->placa ?? 'N/A' }}
                    @if($lavado->cliente->nombre)
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-primary">
                    {{ $lavado->vehiculo->nombre }}
                    <span class="text-text-secondary-dark">(S/ {{ number_format($lavado->vehiculo->precio, 2) }})</span>
                </dd>
            </div>

            <div>
                <dt class="text-xs font-medium text-text-secondary-dark uppercase tracking-wider">Registrado por</dt>
                <dd class="mt-1 text-sm text-primary">{{ $lavado->user->name }}</dd>
            </div>
    ...
    <div class="bg-surface rounded-lg border border-main p-6 mb-6">
        ...
        @if($lavado->trabajadores->isEmpty())
            <p class="text-sm text-secondary">No hay trabajadores asignados.</p>
        @else
            <ul class="space-y-1">
                @foreach($lavado->trabajadores as $trabajador)
    ...
    <div class="bg-surface rounded-lg border border-main p-6 mb-6">
        <div class="flex flex-col items-end gap-2 text-sm">
            @if($lavado->total < $lavado->precio)
                <div class="flex items-center gap-4">
                    <span class="text-secondary">Precio original:</span>
                    <span class="text-primary font-medium">S/ {{ number_format($lavado->precio, 2) }}</span>
                </div>
                <div class="flex items-center gap-4">
                    <span class="text-secondary">Descuento aplicado:</span>
                    <span class="text-primary font-medium">S/ {{ number_format($lavado->precio - $lavado->total, 2) }}</span>
                </div>
                <div class="flex items-center gap-4 border-t border-main pt-2">
                    <span class="text-secondary font-semibold">Total final:</span>
                    <span class="text-primary font-bold text-base">S/ {{ number_format($lavado->total, 2) }}</span>
                </div>
            @else
                <div class="flex items-center gap-4 border-t border-main pt-2">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="text-primary font-medium">{{ $metodosLabel[$lavado->metodo_pago] ?? ucfirst($lavado->metodo_pago) }}</span>
            </div>
            @if($lavado->metodo_pago === 'mixto')
                @if($lavado->monto_efectivo)
                    <div class="flex items-center gap-4 pl-4">
                        <span class="text-text-secondary-dark text-xs">Efectivo:</span>
                        <span class="text-secondary text-xs">S/ {{ number_format($lavado->monto_efectivo, 2) }}</span>
                    </div>
                @endif
                @if($lavado->monto_yape)
                    <div class="flex items-center gap-4 pl-4">
                        <span class="text-text-secondary-dark text-xs">Yape:</span>
                        <span class="text-secondary text-xs">S/ {{ number_format($lavado->monto_yape, 2) }}</span>
                    </div>
                @endif
                @if($lavado->monto_izipay)
                    <div class="flex items-center gap-4 pl-4">
                        <span class="text-text-secondary-dark text-xs">Izipay:</span>
                        <span class="text-secondary text-xs">S/ {{ number_format($lavado->monto_izipay, 2) }}</span>
                    </div>
                @endif
            @endif
        </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        ...
        {{-- Generar ticket --}}
        <a href="{{ route('lavados.ticket', $lavado) }}"
           aria-label="Generar ticket del lavado"
        ...
        {{-- Editar --}}
        <a href="{{ route('lavados.edit', $lavado) }}"
           aria-label="Editar lavado"
           class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-text-primary-dark text-sm font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        {{-- Volver al listado --}}
        <a href="{{ route('lavados.index') }}"
           aria-label="Volver al listado de lavados"
           class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-text-primary-dark text-sm font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
