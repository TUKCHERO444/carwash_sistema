...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-text-primary-dark">Lavados</h1>
        <a href="{{ route('lavados.create') }}"
           aria-label="Nuevo lavado"
    ...
    @if($lavados->isEmpty())
        <div class="text-center py-12 text-gray-500 dark:text-text-secondary-dark text-sm">No hay lavados registrados.</div>
    @else
                    ...
                    @foreach($lavados as $lavado)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-6 py-8 text-sm text-gray-700 dark:text-text-secondary-dark">
                                {{ $lavado->trabajadores->pluck('nombre')->implode(', ') }}
                            </td>
                            <td class="px-6 py-8 whitespace-nowrap text-sm text-gray-700 dark:text-text-secondary-dark">
                                S/ {{ number_format($lavado->precio, 2) }}
                            </td>
                            <td class="px-6 py-8 whitespace-nowrap text-sm text-gray-700 dark:text-text-secondary-dark">
                                S/ {{ number_format($lavado->total, 2) }}
                            </td>
                            <td class="px-6 py-8 whitespace-nowrap">
                                 @php
                                    ...
                                    $metodo  = $lavado->metodo_pago ?? 'efectivo';
                                @endphp
                            ...
                            <td class="px-6 py-8 whitespace-nowrap text-sm flex items-center gap-2">
                                {{-- Ver detalle --}}
                                <a href="{{ route('lavados.show', $lavado) }}"
                                   aria-label="Ver detalle del lavado"
                                ...
                                {{-- Eliminar --}}
                                <form method="POST" action="{{ route('lavados.destroy', $lavado) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            aria-label="Eliminar lavado"
                                            data-confirm="¿Está seguro de eliminar este lavado?"
