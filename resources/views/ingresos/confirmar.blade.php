...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-text-primary-dark">Confirmar Ingreso</h1>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <h2 class="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-2">Resumen del Ingreso</h2>
        <p class="text-sm text-blue-700 dark:text-blue-400"><strong>Placa:</strong> {{ $ingreso->automotor->placa ?? 'N/A' }}</p>
        <div class="text-sm text-blue-700 dark:text-blue-400 mt-1">
            <strong>Servicios asignados:</strong>
            @if($ingreso->servicios->count() > 0)
    ...
    <div class="bg-surface rounded-lg border border-main p-6">
        <form id="form-ingreso" action="{{ route('ingresos.procesarConfirmacion', $ingreso) }}" method="POST" enctype="multipart/form-data" novalidate>
            @csrf
            ...
            <div class="mb-5">
                ...
                <select id="vehiculo_id" name="vehiculo_id" class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors input-main {{ $errors->has('vehiculo_id') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}">
                    ...
                    @foreach($vehiculos as $vehiculo)
                        <option value="{{ $vehiculo->id }}" data-precio="{{ $vehiculo->precio }}" {{ old('vehiculo_id', $ingreso->vehiculo_id) == $vehiculo->id ? 'selected' : '' }}>
            ...
            <div class="mb-5">
                ...
                <input type="text" id="placa" name="placa" required maxlength="7" value="{{ old('placa', $ingreso->automotor->placa ?? '') }}" placeholder="Ej: ABC-123" class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors input-main {{ $errors->has('placa') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}">
            ...
            <div class="mb-5">
                ...
                <input type="date" id="fecha" name="fecha" required value="{{ old('fecha', $ingreso->fecha ? $ingreso->fecha->format('Y-m-d') : date('Y-m-d')) }}" class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors input-main {{ $errors->has('fecha') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}">
        ...
        <form id="form-eliminar" action="{{ route('ingresos.destroy', $ingreso) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
...
window.confirmarMetodoPago = @json($ingreso->metodo_pago ?? 'efectivo');
window.confirmarMontos     = @json($montosData);
window._confirmarUpdateUrl = "{{ route('ingresos.update', $ingreso) }}";
</script>
@vite('resources/js/ingresos/confirmar.js')
<script>
...
<x-modal id="modal-caja-cerrada" title="Caja Cerrada" open>
    ...
    <x-slot:footer>
        ...
        <a href="{{ route('ingresos.index') }}" class="mt-3 sm:mt-0 w-full sm:w-auto inline-flex justify-center rounded-md border border-main shadow-sm px-4 py-2 bg-surface text-base font-medium text-secondary hover:bg-slate-50 dark:hover:bg-slate-700 sm:text-sm">
            Cancelar
        </a>
    </x-slot:footer>
</x-modal>
