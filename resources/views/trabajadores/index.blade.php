...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-text-primary-dark">Trabajadores</h1>
        <a href="{{ route('trabajadores.create') }}"
           aria-label="Crear trabajador"
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-6 py-8 whitespace-nowrap text-sm">
                                <span data-trabajador-id="{{ $trabajador->id }}"
                                      class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $trabajador->estado ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' }}">
                            ...
                            <td class="px-6 py-8 whitespace-nowrap text-sm flex items-center gap-2">
                                ...
                                <button type="button"
                                    ...
                                    {{ $trabajador->estado ? 'Inactivar' : 'Activar' }}
                                </button>

                                <form method="POST" action="{{ route('trabajadores.destroy', $trabajador) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
...
@vite('resources/js/trabajadores/index.js')
@endsection
