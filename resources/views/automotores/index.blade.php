...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-text-primary-dark">Automotores</h1>
        <a href="{{ route('automotores.create') }}"
           aria-label="Crear automotor"
    ...
    @if($automotores->isEmpty())
        <div class="text-center py-12 text-gray-500 text-sm">
            No hay automotores registrados.
        </div>
    @else
                ...
                <tbody class="bg-surface divide-y divide-main">
                    @foreach($automotores as $automotor)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-6 py-8 whitespace-nowrap text-sm text-primary font-medium uppercase">
                                {{ $automotor->placa }}
                            </td>
                            <td class="px-6 py-8 text-sm text-secondary">
                                {{ $automotor->cliente?->nombre_completo ?? '—' }}
                            </td>
                            ...
                            <td class="px-6 py-8 whitespace-nowrap text-sm flex items-center gap-2">
                                <a href="{{ route('automotores.edit', $automotor) }}"
                                   aria-label="Editar automotor {{ $automotor->placa }}"
                                ...
                                <form method="POST" action="{{ route('automotores.destroy', $automotor) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            aria-label="Eliminar automotor {{ $automotor->placa }}"
                                            data-confirm="¿Estás seguro de eliminar este automotor?"
