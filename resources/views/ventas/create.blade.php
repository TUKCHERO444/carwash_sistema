        ...
        <form id="form-venta" action="{{ route('ventas.store') }}" method="POST" novalidate>
            ...
            <div class="mb-6 max-w-sm space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            step="0.01"
                            readonly
                            @error('total') aria-describedby="total-error" aria-invalid="true" @enderror
                    ...
                    @error('total')
                        <p id="total-error" role="alert" class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @error('metodo_pago')
                        <p id="metodo-pago-error" role="alert" class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                <textarea
                    ...
                    @error('observacion') aria-describedby="observacion-error" aria-invalid="true" @enderror
                    class="w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors input-main
                           {{ $errors->has('observacion') ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : '' }}"
                >{{ old('observacion') }}</textarea>
                @error('observacion')
                    <p id="observacion-error" role="alert" class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
...
<x-modal id="modal-caja-cerrada" title="Caja Cerrada" open>
    ...
    <x-slot:footer>
        ...
        <a href="{{ route('ventas.index') }}" class="mt-3 sm:mt-0 w-full sm:w-auto inline-flex justify-center rounded-md border border-main shadow-sm px-4 py-2 bg-surface text-base font-medium text-secondary hover:bg-slate-50 dark:hover:bg-slate-700 sm:text-sm">
            Cancelar
        </a>
    </x-slot:footer>
</x-modal>
